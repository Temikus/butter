...
func TestProviderName(t *testing.T) {
	p := New("", nil)
	if p.Name() != "together" {
		t.Errorf("expected together, got %s", p.Name())
	}
}
...
func TestCustomBaseURL(t *testing.T) {
	...
	resp, err := p.ChatCompletion(context.Background(), &provider.ChatRequest{
		...
		RawBody: []byte(`{"model":"meta-llama/Llama-3.3-70B-Instruct-Turbo"}`),
		APIKey:  "test-key",
	})
	if err != nil {
...
func TestChatCompletionStream(t *testing.T) {
	...
	stream, err := p.ChatCompletionStream(context.Background(), &provider.ChatRequest{
		Model:   "meta-llama/Llama-3.3-70B-Instruct-Turbo",
		Stream:  true,
		RawBody: []byte(`{"model":"meta-llama/Llama-3.3-70B-Instruct-Turbo","stream":true}`),
		APIKey:  "test-key",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
