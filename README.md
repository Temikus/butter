...
The config file supports `${ENV_VAR}` substitution, so the default `config.example.yaml` works out of the box once the environment variables are set. A reference to an unset variable fails the load with an error naming every missing variable. References on fully commented-out lines are ignored; a reference in a trailing comment on a live line is not.
