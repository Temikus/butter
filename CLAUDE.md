...
- `internal/appkey/` — Application key store. Thread-safe in-memory map of `btr_`-prefixed tokens → per-key usage counters (requests, prompt/completion tokens, last_accessed_at) and lifecycle atomics (`expires_at`, `revoked_at`). Async token counting via goroutine. Lifecycle: `Revoke`, `Rotate`, `SetExpiry`, `Delete` (full purge incl. usage history) with `ErrUnknownKey`; lookup-time `IsActive` check; `SetOnUpdate` callback fired on Vend/Revoke/Rotate/SetExpiry; separate `SetOnDelete(func(key string))` callback for purge so persistence layers can evict without overloading the snapshot signal with a tombstone. Streaming SSE token extraction via `AnthropicStreamUsageSink` (line-buffered `io.Writer` parsing `message_start` for `input_tokens` and `message_delta` for cumulative `output_tokens`; never returns errors so it composes safely under `io.MultiWriter`). Structured audit log via `*slog.Logger` on all lifecycle methods (`appkey.vend`, `appkey.revoke`, `appkey.rotate`, `appkey.set_expiry`, `appkey.delete`, `appkey.set_scopes`). Per-key scopes via `atomic.Pointer[KeyScopes]` — optional `allowed_models`/`allowed_providers` restrictions; `VendOption` functional options for `Vend`; `Rotate` inherits scopes; `UpdateScopes` method; context propagation via `WithScopesCtx`/`ScopesFromContext`; enforcement in `Engine.enforceScopes` returns 403 on denied model/provider. Optional bbolt-backed persistence (`persist.go`): write-behind flush on interval + shutdown, immediate write on Vend/Revoke/Rotate/SetExpiry via the `onUpdate` hook, immediate `Delete` on purge via the `onDelete` hook; `writeMu` serializes flush vs. immediate writes to prevent stale snapshots clobbering fresh revokes or resurrecting purged keys. Hot path untouched. Zero overhead when disabled.
...
- `internal/plugin/` — Plugin interfaces (`TransportPlugin`, `LLMPlugin`, `ObservabilityPlugin`), ordered `Chain`, and `Manager`. `RequestContext` carries `ResponseHeaders http.Header` for plugins to set outbound response headers, and `Metadata["_stream_body_sink"]` (generic `io.Writer`) for plugins to capture Anthropic streaming bytes. Built-in plugins: `ratelimit/` (global, per-IP, and per-app-key rate limiting with bucket cleanup), `requestlog/` (dual TransportPlugin+ObservabilityPlugin: per-request correlation ID via configurable `request_id_header`, streaming response body capture via `cappedWriter` with `sync.Pool`, caller identity via `app_key`, request/response body logging with `body_max_bytes` cap), `metrics/` (OTel SDK, Prometheus `/metrics`, optional per-app-key labels), `tracing/` (OTel spans, OTLP HTTP export).
...
**Endpoints:** `POST /v1/chat/completions`, `POST /v1/messages` (Anthropic Messages API — routes to `AnthropicNativeHandler` providers with failover; per-key per-model usage recorded via `ExtractAnthropicUsage` on non-streaming and `AnthropicStreamUsageSink` on streaming, both reading `usage.input_tokens`/`output_tokens`), `POST /v1/embeddings`, `GET /v1/models`, `GET /healthz`, `GET /metrics` (when metrics plugin enabled), `/native/{provider}/*` (raw passthrough). When `app_keys.enabled: true`: `POST /v1/app-keys` (vend key; optional `ttl_seconds`, `allowed_models`, `allowed_providers`), `GET /v1/app-keys` (list keys, includes lifecycle status and scopes), `GET /v1/app-keys/{key}/usage` (per-key stats), `DELETE /v1/app-keys/{key}` (revoke; preserves usage history), `PATCH /v1/app-keys/{key}` (set/clear expiry via `ttl_seconds` or `expires_at`; update scopes via `allowed_models`/`allowed_providers`), `POST /v1/app-keys/{key}/rotate` (vend new key + revoke old; inherits scopes), `POST /v1/app-keys/{key}/purge` (hard-delete; full removal of key and usage history from memory and bbolt), `GET /v1/usage` (aggregate stats).
...
- **Phase 11** (App Key Lifecycle + Anthropic Usage): complete — App-key revoke/rotate/expiry (lifecycle atomics on `UsageRecord`, lookup-time `IsActive`, persistence via `SetOnUpdate` with `writeMu` race fix; `DELETE /v1/app-keys/{key}`, `POST /v1/app-keys/{key}/rotate`, `PATCH /v1/app-keys/{key}`, `ttl_seconds` on create with optional `app_keys.default_ttl` config; middleware rejects inactive keys with 401 regardless of `require_key`); per-model usage tracking on `/v1/messages` (`ExtractAnthropicUsage` for `usage.input_tokens`/`output_tokens`, async record on non-streaming + streaming branches — streaming branch recorded `(0, 0)` until Phase 12)
- **Phase 12** (App Key Purge + Streaming Token Extraction): complete — `POST /v1/app-keys/{key}/purge` endpoint backed by `Store.Delete` + `SetOnDelete` hook + `Persister.DeleteKey` (resolves the "zombie keys reappearing on restart" gap left by upsert-only flush); `AnthropicStreamUsageSink` tee'd via `io.MultiWriter` on the streaming `/v1/messages` branch captures `input_tokens` from `message_start` and cumulative `output_tokens` from `message_delta`, replacing the prior zero-record without adding latency to the client-facing flush path; integration test polls `/v1/app-keys/{key}/usage` rather than `time.Sleep` to avoid flake on async record; cleanup of empty `internal/keyring/` scaffold; PRD provider table synced to include Azure OpenAI
- **Phase 13** (Audit Log + Per-Key Scopes): complete — Structured audit log via `*slog.Logger` on all app-key lifecycle methods (`appkey.vend`, `appkey.revoke`, `appkey.rotate`, `appkey.set_expiry`, `appkey.delete`, `appkey.set_scopes`); per-key scopes (`KeyScopes` with `allowed_models`/`allowed_providers`, `atomic.Pointer` for lock-free hot-path reads, `VendOption` functional options, `Rotate` inherits scopes, `UpdateScopes` method); scope enforcement in `Engine.enforceScopes` (model denied → 403, provider filter → reduced provider list, all providers denied → 403); context propagation via `WithScopesCtx`/`ScopesFromContext`; API: `POST /v1/app-keys` accepts scopes, `PATCH /v1/app-keys/{key}` updates scopes, config `AppKeyEntry` supports `allowed_models`/`allowed_providers`; README synced (11 providers, Azure OpenAI live, feature list updated)
- **Phase 14** (OpenAI Streaming Token Extraction + PRD Hygiene): complete — `OpenAIStreamUsageSink` extracts `usage.prompt_tokens`/`usage.completion_tokens` from OpenAI-format SSE streaming chunks via `FeedChunk` pull-model API (the `/v1/chat/completions` streaming path uses `stream.Next()`, not `io.Copy`); `InjectStreamOptions` transparently adds `stream_options.include_usage=true` to outbound streaming requests when app-key tracking is active (uses `map[string]json.RawMessage` for safe JSON manipulation, ~3μs overhead); both wired into `handleStream` with conditional activation guard (only when `appKeyStore != nil` and key present in context); replaces prior `(0, 0)` zero-record for all 9 OpenAI-compatible providers; integration test with conditional mock validates end-to-end pipeline; PRD hygiene pass: removed stale `keyring/`/`observe/` from project structure, fixed `Provider` interface code block (removed phantom `Embeddings`/`Models` methods, corrected `Stream.Next()` return type), added missing directories (`appkey/`, `version/`, `azureopenai/`, `prompt-injection-guard/`, `redis.go`), deleted empty `internal/observe/` directory
- **Next**: GitHub Copilot provider (#23, spec complete in `design/backlog.md`), Vertex AI, semantic cache plugin, or other Bifrost-roadmap providers (dynamic injection-guard signatures also parked in `design/backlog.md`)
