...
**Endpoints:** `POST /v1/chat/completions`, `POST /v1/messages` (Anthropic Messages API — routes to `AnthropicNativeHandler` providers with failover; per-key per-model usage recorded via `ExtractAnthropicUsage` on non-streaming and `AnthropicStreamUsageSink` on streaming, both reading `usage.input_tokens`/`output_tokens`), `POST /v1/embeddings`, `GET /v1/models`, `GET /healthz`, `GET /metrics` (when metrics plugin enabled), `/native/{provider}/*` (raw passthrough). When `app_keys.enabled: true`: `POST /v1/app-keys` (vend key; optional `ttl_seconds`), `GET /v1/app-keys` (list keys, includes lifecycle status), `GET /v1/app-keys/{key}/usage` (per-key stats), `DELETE /v1/app-keys/{key}` (revoke; preserves usage history), `PATCH /v1/app-keys/{key}` (set/clear expiry via `ttl_seconds` or `expires_at`), `POST /v1/app-keys/{key}/rotate` (vend new key + revoke old), `POST /v1/app-keys/{key}/purge` (hard-delete; full removal of key and usage history from memory and bbolt), `GET /v1/usage` (aggregate stats).
